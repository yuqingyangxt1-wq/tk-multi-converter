...
from .tiktok_writer import template_path_for
...
class App(tk.Tk):
    ...
    def _build_chrome(self):
        # Country selector — top-of-window radio that switches ID/PH/TH.
        # Triggers _on_country_change which updates window title, rebuilds
        # the settings panel (some settings differ per country), and writes
        # the new country to config.json.
        self._var_country = tk.StringVar(value=self.cfg.get("country", "PH"))
        country_bar = tk.Frame(self, bg="#f4f4f4", height=36)
        country_bar.pack(side="top", fill="x")
        tk.Label(
            country_bar, text="目标国家:",
            bg="#f4f4f4", fg="#222",
            font=("", 10, "bold"),
        ).pack(side="left", padx=(16, 8), pady=6)
        for code, name in cfg_mod.country_choices():
            tk.Radiobutton(
                country_bar,
                text=name,
                variable=self._var_country,
                value=code,
                command=self._on_country_change,
                bg="#f4f4f4", fg="#222",
                activebackground="#f4f4f4",
                font=("", 10),
                indicatoron=0,  # button-style radio (more obvious)
                padx=12, pady=2,
                selectcolor="#0a6",

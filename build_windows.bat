...
set "APP_NAME=TK-Multi-Converter"
set "ROOT=%~dp0"
pushd "%ROOT%"
...
echo.
echo === [6/6] Copying country templates to dist ===
for %%c in (id ph th) do (
  if not exist "dist\assets-%%c" mkdir "dist\assets-%%c"
  if exist "assets-%%c\batch-product-source.xlsx" (
    copy /Y "assets-%%c\batch-product-source.xlsx" "dist\assets-%%c\" >nul
    echo   Copied assets-%%c\batch-product-source.xlsx
  )
  if exist "assets-%%c\default_size_chart.png" (
    copy /Y "assets-%%c\default_size_chart.png" "dist\assets-%%c\" >nul
  )
)
echo   All three country templates copied
...
echo   Templates:   dist\assets-id\, dist\assets-ph\, dist\assets-th\
echo.
echo   Next steps:
echo     1. Open the dist\ folder
echo     2. Double-click %APP_NAME%.exe to run
echo     3. Pick ID / PH / TH at the top of the window
echo     4. To distribute: zip the entire dist\ folder
echo ============================================================
echo.
pause
popd
